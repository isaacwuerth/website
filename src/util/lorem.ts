...
const sentences = indexes(words);
sentences.unshift(words[0]);
sentences.pop();

export default function lorem(wordcount: number): string {
  const random = Math.floor(Math.random() * sentences.length);
  const max = words.length - wordcount;

  if (random > max) {
    lorem(wordcount);
  }

  const start = sentences[random];
  const pos = words.indexOf(start);
  const output = [];
